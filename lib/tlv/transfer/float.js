...
class FloatParse extends BaseTransfer {
  ...
  binaryToType(binary) {
    ...
    assert(binary.length === 4, `[FloatParse][binaryToType] - 长度非4字节，binary length: ${binary.length}`);
  ...
  typeToBinary(value) {
    ...
    if (Buffer.isBuffer(value)) {
      assert(value.length === 4, `[FloatParse][typeToBinary] - 长度非4字节，value length: ${value.length}`);
    }
