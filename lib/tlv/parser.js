...
class Parser {
  ...
  parse(buffer) {
    ...
    try {
      ...
      params = operations.type === 'device' ? this.getFunctionPayloadInfo(index, functionPayload) : this.getSubdevicePayloadInfo(operations.code, index, functionPayload); // 获取功能点及功能点值信息
   ...
   * @memberof Parser
   */
  getSubDeviceDevisionNumber(code) {
    let number = null;
    if ([ 0x7f, 0x7e, 0x64, 0x65 ].includes(code)) number = 2;
    if ([ 0x62, 0x63 ].includes(code)) number = 3;
    return number;
  }
   ...
   * @memberof Parser
   */
  getSubdevicePayloadInfo(code, index, buffer) {
    thingAssert(buffer.length > 2, '数据错误，缺少功能点值数据', 400);
    const number = this.getSubDeviceDevisionNumber(code);
    if (number) return this.getSubdevieCustomPayloadInfo(index, buffer, number);
  ...
  getSubDevicePayloadInfo(index, buffer) {
    ...
    params.forEach((item, index) => {
      ...
      const isPidLable = functionId === 0xe801 && index !== 0;
      const isTimeLable = functionId === 0xe803;
      ...
      if (isPidLable || isLastItem) {
        devidedParams.push(newGroup);
        newGroup = {};
        newGroup[functionId] = item;
      }
    });
   ...
   * @memberof Parser
   */
  getSubdevieCustomPayloadInfo(index, buffer, number) {
    const bufferLength = buffer.length;
    const params = [];
    let subDeviceInfo = {};
    let count = 0;
    while (index + this[THING_FIXED_PAYLOAD_LENGTH] <= bufferLength) {
      ...
      if (index + valueLength <= bufferLength) {
        ...
        index += valueLength;
        value = this.transfromTlvToObject({
          dataType: 'string',
        }, valueBuffer);
      }
  ...
  getUnifiedResponse(data) {
    ...
    let groupId = null;
    const params = isSubdevice ? paramsList : {};
