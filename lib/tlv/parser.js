...
class Parser {
  ...
  packageSplit(buffer) {
    ...
    assert(versionVal !== 0 && versionVal !== 128, '[Parser][packageSplit] - 版本号不能为0');
    const hasMsgId = versionVal > 128; // 是否包含消息id
    let version = null;
    version = !hasMsgId ? versionVal : versionVal - 128; // 获取真实版本号
    const msgId = hasMsgId ? buffer.readUInt32BE(this[TLV_VERSION_LENGTH]) : null; // 获取消息id
    ...
    if (crcCode !== crcPayload) {
      this.app.logger.error(`[Parser][packageSplit] - 数据错误,crc校验失败,预期:${crcPayload},实际:${crcCode},buffer:`, buffer);
      return;
    }

    buffer = buffer.slice(this[TLV_METHOD_LENGTH] + this[TLV_VERSION_LENGTH] + (hasMsgId ? this[TLV_MSGID_LENGTH] : 0), bufferLength - 2); // 获取buffer payload
    const index = 0; // 解析索引
