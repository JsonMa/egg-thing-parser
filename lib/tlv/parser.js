    ...
    while (index + this[TLV_FIXED_LENGTH] <= bufferLength) {
      ...
      if (!valueLength) {
        ...
        if (bigLength > 63 && bigLength <= 127) { // eslint-disable-line
          valuePrefixLength = 2;
          valueLength = buffer.readUInt16BE(index + this[TLV_FIXED_LENGTH]) - 16384; // 截取最高位01
        } else if (bigLength > 127 && bigLength <= 191) {
          valuePrefixLength = 3;
          valueLength = buffer.readUIntBE(index + 2, 3) - 8388608; // 截取最高位10
        } else if (bigLength > 191) {
          valuePrefixLength = 4;
          valueLength = buffer.readUInt32BE(index + this[TLV_FIXED_LENGTH]) - 3221225472; // 截取最高位10
        } else {
