...
class Parser {
  ...
  packageSplit(buffer) {
    ...
    assert(bufferLength >= 7, `[Parser][packageSplit] - 数据错误，长度至少需要7个字节，当前数据长度: ${bufferLength}`);
    ...
    assert.deepEqual(crcCode, crcPayload, '[Parser][packageSplit] - 数据错误：CRC校验失败');
...
s   * @return {Array}  转换结果
  ...
  bufferConvert(index, buffer, params = []) {
    ...
    while (index + this[TLV_FIXED_LENGTH] <= bufferLength) {
      ...
      // 获取value 长度
      if (!valueLength) {
        const bigLength = buffer.readUIntBE(index + this[TLV_FIXED_LENGTH], this[TLV_VALUE_FIXED_LENGTH]);
        if (bigLength & 0x80 === 0) { // eslint-disable-line
          valueLength = buffer.readUIntBE(index + this[TLV_FIXED_LENGTH], this[TLV_VALUE_FIXED_LENGTH]);
          valuePrefixLength = 2;
        } else {
      ...
      index += functionInfo.valueLength ? this[TLV_FIXED_LENGTH] : this[TLV_FIXED_LENGTH] + valuePrefixLength;
