...
class Parser {
   ...
   */
  packageSplit(buffer) {
    const methodValue = buffer.readUIntBE(0, this[TLV_METHOD_LENGTH]); // 获取取操作码
    const method = this.methodConvert(methodValue);
    const bufferLength = buffer.length;
    buffer = buffer.slice(1, bufferLength - 2); // 获取buffer payload
    const index = 0; // 解析索引
  ...
  bufferConvert(index, buffer) {
    ...
    while (index + this[TLV_FIXED_LENGTH] <= bufferLength) {
      ...
      if (param.resourceType === 'combine') param.value = this.bufferConvert(index, value);
      params.push(param);
    }
    return params;
  }
  ...
  unifyResponse(data) {
    ...
    return {
      version: this.app.config.thingParser.thing_version,
      method: data.method,
      params,
    };
  }
