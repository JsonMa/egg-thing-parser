...
class Parser {
  ...
  parse(buffer) {
    ...
    try {
      ...
      if (isResponseOperation) {
        ...
        const index = 0; // 解析索引
        const functionPayload = buffer.slice(this[THING_VERSION_LENGTH] + this[THING_METHOD_LENGTH] + (hasMsgId ? this[THING_MSGID_LENGTH] : 0), bufferLength - this[THING_CRC_LENGTH]);
  ...
  transfromTlvToObject(functionInfo, functionValueBuffer) {
    ...
    return this.app.thing.tlv.transfer[dataType].binaryToType(functionValueBuffer);
  }
  ...
  getDataType(dataTypeCode) {
    thingAssert(dataTypeCode !== 0, '[Parser][getDataType] - 数据错误，类型不能为0', 0x40);
    let dataType = null;
