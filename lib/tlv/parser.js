...
class Parser {
  ...
  binaryConvert(data) {
    ...
    return data.map(item => {
      ...
      assert(
        ...
        `[Parser][binaryConvert] - 数据类型错误：type：${item.type}`
      );
  ...
  arrayConvert(data) {
    ...
    bufferArr = data.map(item => {
      assert(
        item.type && typeof item.value !== 'undefined',
        `[Parser][arrayConvert] - 参数错误: type: ${item.type}, value: ${item.value}`
      );
  ...
  packageSplit(buffer) {
    ...
    assert(
      bufferLength >= 7,
      `[Parser][packageSplit] - 数据错误，长度至少需要7个字节，当前数据长度: ${bufferLength}`
    );
    const crcPayload = buffer.readUInt16BE(bufferLength - 2);
    const payload = buffer.slice(0, bufferLength - 2);
    const crcCode = crc16ccitt(payload);
    if (crcCode !== crcPayload) {
      this.app.logger.error(`[Parser][packageSplit] - 数据错误,crc校验失败,预期:${crcPayload},${crcCode},buffer:`, buffer);
      return;
    }
...
s   * @return {Array}  转换结果
  ...
  bufferConvert(index, buffer, params = []) {
    ...
    while (index + this[TLV_FIXED_LENGTH] <= bufferLength) {
      ...
      // 获取value 长度
      if (!valueLength) {
        const bigLength = buffer.readUIntBE(
          index + this[TLV_FIXED_LENGTH],
          this[TLV_VALUE_FIXED_LENGTH]
        );
        if (bigLength & (0x80 === 0)) {
          // eslint-disable-line
          valueLength = buffer.readUIntBE(
            index + this[TLV_FIXED_LENGTH],
            this[TLV_VALUE_FIXED_LENGTH]
          );
          valuePrefixLength = 2;
      ...
      index += functionInfo.valueLength ?
        this[TLV_FIXED_LENGTH] :
        this[TLV_FIXED_LENGTH] + valuePrefixLength;
  ...
  dataTypeConvert(number) {
    assert(
      number <= 7,
      `[Parser][messageTypeConvert] - 数据类型错误，长度超出3位，number: ${number}`
    );
  ...
  unifyResponse(data) {
    ...
    data.params.forEach(param => {
      ...
      params[param.resourceId] = {
        value,
        type: param.dataType,
        message_type: param.messageType,
