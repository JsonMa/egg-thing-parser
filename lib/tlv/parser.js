...
class Parser {
  ...
  parse(buffer) {
    ...
    thingAssert(
      ...
      `[Parser][parse] - 数据非buffer类型: ${buffer}`,
      0x40
    ); // 数据类型校验
    ...
    thingAssert(
      versionMsgIdValue !== 0 && versionMsgIdValue !== 128,
      '[Parser][parse] - 版本号不能为0',
      0x40
    ); // 首字节校验
    ...
    const baseLengthRequired = hasMsgId ? 9 : 5; // 消息基础长度需求
    thingAssert(
      bufferLength >= baseLengthRequired,
      `[Parser][parse] - 数据错误，长度至少需要${baseLengthRequired}个字节，当前数据长度: ${bufferLength}`,
      0x41
    );
    ...
    const lengthRequired =
      operations.operation === 'response'
        ? baseLengthRequired
        : baseLengthRequired + 2;
    thingAssert(
      bufferLength >= lengthRequired,
      `[Parser][parse] - 数据错误，长度至少需要${lengthRequired}个字节，当前数据长度: ${bufferLength}`,
      0x41
    );

    // CRC校验
    const crcPayload = buffer.readUInt16BE(
    ...
    thingAssert(
      crcCode === crcPayload,
      `[Parser][parse] - 数据错误, CRC校验失败, 预期:${crcPayload}, 实际:${crcCode}`,
      0x42
    );
    ...
    try {
      ...
      if (isResponseOperation) {
        code = buffer.readUInt8(
          hasMsgId
            ? this[THING_MSGID_LENGTH] +
                this[THING_VERSION_LENGTH] +
                this[THING_METHOD_LENGTH]
            : this[THING_VERSION_LENGTH] + this[THING_METHOD_LENGTH]
        ); // 获取响应值
      }
      const index = 0; // 解析索引
      const functionPayload = buffer.slice(
        this[THING_VERSION_LENGTH] +
          this[THING_METHOD_LENGTH] +
          (hasMsgId ? this[THING_MSGID_LENGTH] : 0),
        bufferLength - this[THING_CRC_LENGTH]
      );
  ...
  getFunctionPayloadInfo(index, buffer, params = []) {
    ...
    while (index + this[THING_FIXED_PAYLOAD_LENGTH] <= bufferLength) {
      const functionBuffer = buffer.slice(
        index,
      ...
      if (!valueLength) {
        ...
        if (bigLength > 63 && bigLength <= 127) {
          ...
          valuePrefixLength = 2;
          valueLength =
            buffer.readUInt16BE(index + this[THING_FIXED_PAYLOAD_LENGTH]) -
            16384; // 截取最高位01
          ...
          valuePrefixLength = 4;
          valueLength =
            buffer.readUInt32BE(index + this[THING_FIXED_PAYLOAD_LENGTH]) -
            3221225472; // 截取最高位10
        } else {
      ...
      index += functionInfo.valueLength
        ? this[THING_FIXED_PAYLOAD_LENGTH]
        : this[THING_FIXED_PAYLOAD_LENGTH] + valuePrefixLength;
      if (index + valueLength <= bufferLength) {
        valueBuffer = buffer.slice(index, index + valueLength); // 功能点值buffer
        index += valueLength;
      }
      const value = this.transfromTlvToObject(functionInfo, valueBuffer);
  ...
  getUnifiedResponse(data) {
    const { code, id, operations, params: paramsList, version } = data;
    let groupId = null;
    const params = {};
