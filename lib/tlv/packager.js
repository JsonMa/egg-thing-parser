...
class Packager {
  ...
  package(params) {
    ...
    const hasMsgId = !!id;
    const isSubDeviceNotify = !!(type === 'subDevice' && method === 'notify');
    const isResponse = operation === 'response' || operationCode > 127;
    const isReadRequest = method === 'read' && !isResponse;
    if (isReadRequest) thingAssert(functionInfos.length === 1, '参数错误，read请求时功能点数量只能为1', 400); // 校验read 请求
    ...
    functionInfos.forEach(functionInfo => {
      ...
      if (isSubDeviceNotify && subDeviceGroupId) {
        ...
        [ functionPayloadLength, functionPayload ] = this.commonFunctionProcessor(functionInfo, method, isResponse);
      }
    ...
    if (groupId) {
      ...
      payloadBuffer = Buffer.concat([ payloadBuffer, groupPayload ], payloadLength);
    }
   ...
   * @param {boolean} isResponse     - 是否为响应
   * @return {array} payload
   * @memberof Packager
   */
  commonFunctionProcessor(functionInfo, method, isResponse) {
    const {
      valueType,
      functionId,
      value,
