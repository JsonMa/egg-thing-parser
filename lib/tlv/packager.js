...
class Packager {
  ...
  package(params) {
    ...
    const { version, method, code, data, id, operations } = params;
    let { code: operationCode, method, target, type, operation } = operations;
    const { params: functionInfos, groupId } = data;
    const hasMsgId = !!id;
    ...
    // 响应码
    let responseCodeBuffer = Buffer.allocUnsafe(
    ...
    functionInfos.forEach(item => {
      const { valueType, functionId, value } = item;
  ...
  getOperationValue(method, target, type, operation) {
    let methodCode = null;
    const targetCode = ['resource', 'system'].includes(target);
    const typeCode = ['device', 'subDevice'].includes(type);
    const operationCode = ['request', 'response'].includes(operation);
    if (target === 'system') {
      if (!['online', 'offline'].includes(method)) {
        methodCode = method === 'onlie' ? 0x1f : 0x1e;
      } else {
        methodCode = [
          'reset',
          'recovery',
      ...
      methodCode = ['read', 'write', 'notify'].indexOf(method);
    }
    methodCode = methodCode.toString(2); // 转换为二进制字符串
