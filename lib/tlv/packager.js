...
class Packager {
  ...
  package(params) {
    ...
    const isSubDeviceWriteResponse = type === 'subDevice' && isResponse;
    ...
    functionInfos.forEach(functionInfo => {
      ...
      if (subDeviceGroupId) {
        ...
        [ functionPayloadLength, functionPayload ] = this.commonFunctionProcessor(functionInfo, isReadRequest, isSubDeviceReadRequest || isSubDeviceWriteResponse);
      }
   ...
   * @param {boolean}  isSubDeviceReadRequest  - 是否网关子设备read请求
   * @return {array} payload
   * @memberof Packager
   */
  commonFunctionProcessor(functionInfo, isReadRequest, isSubDeviceReadRequest) {
    const {
      valueType,
      functionId,
      value,
    ...
    if (isReadRequest || (isSubDeviceReadRequest && !value)) {
